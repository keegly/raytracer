...
public:
	...
	virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered) const {
		...
		float reflect_prob;
		float cosine;
		std::uniform_real_distribution<float> drand(-1.0, 1.0);
		...
		if (refract(r_in.direction(), outward_normal, ni_over_nt, refracted)) {
			...
			// (note: the first appearance of “scattered =” above is not needed and in fast is always 
			// over-written later. Doesn’t affect result but is a performance bug).
			// scattered = ray(rec.p, reflected);
			reflect_prob = 1.0;
		}
		if (drand(generator) < reflect_prob) {
			scattered = ray(rec.p, reflected);
		}
		else {
...
int main()
{
	...
	list[0] = new sphere(vec3(0, 0, -1), 0.5, new lambertian(vec3(0.1, 0.2, 0.5))); // Blue middle
	list[1] = new sphere(vec3(0, -100.5, -1), 100, new lambertian(vec3(0.8, 0.8, 0.0))); // Giant green that acts as ground
